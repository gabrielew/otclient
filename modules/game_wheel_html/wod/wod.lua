...
local function resetSlotSelection(ui)
    for quadrantKey, _ in pairs(QuadrantSlotConfig) do
...
function WheelOfDestiny.show(container)
    ...
    local wodUI = container:recursiveGetChildById('wodContainer')
    if not wodUI then
        return
    end

    currentWodUI = wodUI
    wodUI:setVisible(true)
    wodUI:fill('parent')
    ...
    if not wodInitialized then
        UIConfigurator.setupSlotSelection(wodUI)
    else
        resetSlotSelection(wodUI)
    end
