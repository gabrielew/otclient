...
function toggle()
    ...
    local canAccess, errorMsg = WheelOfDestiny.canAccessWheel()
...
function controllerWheel:onInit()
    self:loadHtml('game_wheel_html.html')
    if self.ui then
        self.ui:hide()
        contentContainer = self.ui:recursiveGetChildById('contentContainer')
        local buttonContainer = self.ui:recursiveGetChildById('buttonContainer')
...
function controllerWheel:selectWindow(tab)
    SelectWindow(tab, false)
end

function controllerWheel:toggleWindow()
    toggle()
end

function controllerWheel:goBack()
    toggleBack()
end
...
function resetWheelTabs()
    ...
    if contentContainer then
        local panels = { 'wodContainer', 'gemContainer', 'fragContainer' }
        for _, panelId in ipairs(panels) do
            local panel = contentContainer:recursiveGetChildById(panelId)
            if panel then
...
function SelectWindow(type, isBackButtonPress)
    ...
    if contentContainer then
        local panels = {
            wheelOfDestiny = 'wodContainer',
            gemAtelier = 'gemContainer',
            fragmentWorkshop = 'fragContainer'
        }

        for key, panelId in pairs(panels) do
            local panel = contentContainer:recursiveGetChildById(panelId)
            if panel then
                panel:setVisible(key == type)
            end
        end
    end
