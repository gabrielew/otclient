...
local function forEachStatsBar(callback)
    local player = g_game.getLocalPlayer()
    if not player then
        return
    end
...
local function showMonkStats()
    local isMonk = g_game:getLocalPlayer():getVocation() == VocationsClient.Monk
...
function StatsBar.reloadCurrentStatsBarHarmony(localPlayer, harmonies, oldHarmonies)
    ...
    currentHarmony = math.floor(currentHarmony or 0)

    forEachStatsBar(function(barElement)
        if barElement.harmonies then
            local childCount = barElement.harmonies:getChildCount()
            local filledHarmony = math.max(math.min(currentHarmony, childCount), 0)
            for i = 1, childCount do
                ...
                child:setOn(i <= filledHarmony)
            end
        end
    end)
end

function StatsBar.reloadCurrentStatsBarSerene(localPlayer, state)
    forEachStatsBar(function(barElement)
...
function StatsBar.init()
    ...
    statsBarDeepInfo = {
        ...
        onHarmonyChange = StatsBar.reloadCurrentStatsBarHarmony,
        onSereneChange = StatsBar.reloadCurrentStatsBarSerene,
    }
